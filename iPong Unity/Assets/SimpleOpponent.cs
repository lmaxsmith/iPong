...
namespace DefaultNamespace
{
	...
	public class SimpleOpponent : AIControl
	{
		protected override async UniTask Control()
		{
			if (Ball)
			{
				if (IsOutOfThreshold() && IsApproaching())
				{
					if(isLeft())
					{
		...
		public bool isLeft() => TForm.InverseTransformPoint(Ball.TForm.position).x < 
		                        _paddle.TForm.localPosition.x - threshold;
		public bool isRight() => TForm.InverseTransformPoint(Ball.TForm.position).x > 
		                         _paddle.TForm.localPosition.x + threshold;
